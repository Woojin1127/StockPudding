...
### 2026-07-08 (v3.0.0) — 첫 화면 콘텐츠: 급등락·거래대금·인기·푸딩 (`feat/web-v3`)

인베스팅닷컴처럼 "처음 들어왔을 때 볼 게 많은" 메인 페이지 (사용자 요청 1번+4번).

**데이터 소스 조사 결과** (중요):
- pykrx 전종목 스냅샷(`get_market_ohlcv(date)`)은 KRX 로그인 요구로 **막힘**
- 네이버 랭킹 API(`/api/stocks/up|down/...`)는 동작하나 **장 시작 전(PREOPEN)엔 빈 값**, 신고가 엔드포인트는 404
- ✅ **FDR `StockListing('KRX')`가 정답**: 최근 거래일 종가·등락률·거래대금·시총 전종목 포함, 장외 시간에도 동작, 이미 검색용으로 쓰던 의존성
- 52주 신고가 목록은 싼 소스가 없어 백로그로

**무엇을/어떻게**:
- 엔진 `GET /market/movers`: FDR 스냅샷에서 급등/급락/거래대금 TOP5. 잡주 필터(코스피·코스닥, 시총 1000억↑, 스팩 제외). 리스팅 캐시를 TTL 10분으로 변경 — "재시작 전까지 종목 목록 안 갱신" 백로그도 해결
- 랜딩 `MarketSections`: ⚡급등/급락(토글) · 💰거래대금 TOP · 👀오늘 많이 본 종목 · 🏆도움된 분석 TOP 4카드 그리드. 로딩 스켈레톤, 실패/빈 데이터 시 안내 문구
- F8용 `stock_views` 테이블 신설 (`supabase/migration_v3.sql` — **기존 프로젝트는 이 파일 1회 실행 필요**). 분석 페이지 진입 시 조회 기록, 자정 기준 집계
- F7 도움된 분석 TOP: votes를 reports 임베딩으로 조회해 클라이언트 집계 (좋아요순, 트래픽 커지면 뷰/RPC로 전환)
- 🍮 오늘의 푸딩(4번): 점수를 푸딩 상태로 — 65+ 탱글탱글(바운스), 45~64 흔들흔들(워블), 45↓ 와르르(🫠). ScoreCard 게이지 아래 배지
- 검증: movers API 응답 정상(상한가 +30% 등), 랜딩 4카드 렌더+등락색 관례 준수, 투표 1건 실투표→TOP 반영 확인, build/lint/test 통과
